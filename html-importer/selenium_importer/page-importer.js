...
(function() {
  window.importPage = function() {
    var DEBUG, blocks, clientRect, colorIsVisible, doc, getCSSValueUrl, hasVisibleText, isOffscreen, numOfPx, relativeNumOfPx, urlpath;
    doc = {};
    ...
    blocks = $("*").toArray().map($).map(function(domBlock) {
      var block, btype, height, i, left, len, prop, rect, ref, ref1, ref2, ref3, ref4, styles, top, width;
      ...
      ref1 = [rect.height, rect.width], height = ref1[0], width = ref1[1];
      ...
      if (domBlock[0].tagName === 'INPUT' && domBlock[0].type === 'text') {
        btype = 'text-input';
        block.placeholder = domBlock[0].placeholder;
      } else if (((ref3 = domBlock.children()) != null ? ref3.length : void 0) === 0 && domBlock.text() && hasVisibleText(domBlock[0])) {
        if (block.width <= 1) {
          return null;
        }
        btype = 'text';
    ...
    doc.blocks = blocks.filter(function(b) {
      return b !== null;
    });
    return doc;
  };
